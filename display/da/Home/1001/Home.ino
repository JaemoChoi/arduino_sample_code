...
void loop() {
  ...
  switch (SCENE_NO) {
    ...
    case 1:
      PC_COMMAND.println(SCENE_NO);
      digitalWrite(PDLC, LOW);    // PDLC ON
    //  setAll(0, 0, 0, 0);

      //for (int dim = 0 ; dim < 64 ; dim ++) {
        analogWrite(TOP_FRONT, 64);
        analogWrite(TOP_MID, 64);
        analogWrite(DRAW_EDGE, 64);
        analogWrite(DRAW_BACK, 255);
     //   delay(5);
     // }
    ...
    case 2:
      ...
      /*
        ...
        analogWrite(TOP_FRONT, dim);
        analogWrite(TOP_MID, dim);
        analogWrite(DRAW_EDGE, dim);
        analogWrite(DRAW_BACK, dim);
        delay(5);
        }
      */
      SCENE_NO = 255;
    ...
    case 4:
      ...
      analogWrite(TOP_MID, 64);
      analogWrite(DRAW_EDGE, 64);
      analogWrite(DRAW_BACK, 64);
      delay(1200);
      DEZIN_COMMAND.write(ICE);
      SCENE_NO = 255;
      break;
    ...
    case 6:
      ...
      digitalWrite(PDLC, LOW);    // PDLC ON
      setAll(0, 0, 0, 255);       // Back Light ON
      //LIGHT_SCENE_6();
      SCENE_NO = 255;
      break;
