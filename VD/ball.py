...
# 설정 파일 경로
CONFIG_FILE = r"C:\Ballie\config.txt"
...
# VLC 경로
FFPLAY_PATH = r"C:\Ballie\ffmpeg-2025-09-18-git-c373636f55-full_build\bin\ffplay.exe"
# 영상 파일 경로
VIDEO_MONITOR1 = {i+1: f"C:\Ballie\Video/M{i+1}.mp4" for i in range(10)}
VIDEO_MONITOR2 = {i+1: f"C:\Ballie\Video/S{i+1}.mp4" for i in range(10)}
    ...
    while True:
        if ser.in_waiting:  # 시리얼 데이터가 수신되었는지 확인
            ...
            if line.lower() == "1":
                ...
                play_with_ffplay(VIDEO_MONITOR2[1], 2560, 0, 400, 400, mute=True, rotate=0)
            elif line.lower() == "9":
                #print("▶ play 실행: 1_2.mp4 + 2_2.mp4")
                play_with_ffplay(VIDEO_MONITOR1[2], 0, 0, 2560, 1660, mute=False, rotate=0)
                play_with_ffplay(VIDEO_MONITOR2[2], 2560, 0, 400, 400, mute=True, rotate=0)
